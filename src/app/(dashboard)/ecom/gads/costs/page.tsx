...
/**
 * Analytics — Réglages coûts (Phase 1A).
 * Trois sections : COGS par produit (marge brute calculée en live pendant la
 * saisie), frais par commande, dépenses récurrentes.
 * Ces coûts alimentent BE-ROAS (variables uniquement) et Net Profit (tout).
 */
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[20px] font-bold text-[#1a1535]">Réglages coûts</h1>
          <p className="text-[12px] text-[#8A8A88]">
            Le socle de la rentabilité réelle : COGS par produit, frais par commande, dépenses récurrentes
          </p>
        </div>
        <GadsTabs />
      </div>
...
function ProductCostRow({ product: p, onSaved }: { product: ProductRow; onSaved: () => void }) {
  ...
  // Marge brute calculée EN LIVE pendant la saisie (exigence validée).
    ...
    <tr className="border-b border-[#EFEFEF] hover:bg-[#FBFBFA]">
      <td className="px-3 py-2">
        <div className="flex items-center gap-2.5 min-w-[220px]">
          {p.image_url ? (
            // eslint-disable-next-line @next/next/no-img-element -- CDN Shopify, même précédent que CreativesTable
