...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   PANE HEADER — shared component for consistent alignment
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

function PaneHeader({ title, subtitle, action }: { title: string; subtitle: string; action: React.ReactNode }) {
  return (
    <motion.div
      className="flex items-start justify-between mb-5 gap-3"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35 }}
    >
      <div className="min-w-0">
        <h2 className="text-lg font-bold text-[#1A1A1A] leading-tight">{title}</h2>
        <p className="text-[12px] text-[#8A8A88] mt-1">{subtitle}</p>
      </div>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-xl border border-[#E6E6E4] shadow-lg z-50 p-6"
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-[16px] font-semibold text-[#1A1A1A]">Nouveau produit</h2>
        ...
        {briefTemplates.length > 0 && (
          <div className="mt-4">
            ...
            <select value={selectedBriefId} onChange={(e) => setSelectedBriefId(e.target.value)} className={INPUT_CLASS}>
              ...
              {briefTemplates.map((b) => (
                <option key={b.id} value={b.id}>{b.name} (v{b.version} — {(b.schema || []).length} champs)</option>
              ))}
            </select>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   PRODUCT CARD — compact row layout for 50% pane
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

function ProductCard({ product, index, onDuplicate, onArchive }: {
  ...
  return (
    <motion.div
      className="bg-white rounded-xl border border-[#E6E6E4] overflow-hidden hover:shadow-sm hover:border-[#D0D0CE] transition-all group"
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, delay: index * 0.03, ease: "easeOut" as const }}
    >
      {/* Horizontal card: thumbnail + info */}
      <div className="flex">
        {/* Thumbnail */}
        ...
        <div className="flex-1 min-w-0 p-3.5 flex flex-col justify-between">
          <div>
            ...
            {product.shortDescription && (
              <p className="text-[11px] text-[#8A8A88] line-clamp-1 mt-0.5">{product.shortDescription}</p>
            )}
          </div>
      ...
      <div className="border-t border-[#E6E6E4] px-3 py-1.5 flex items-center">
        <button onClick={() => router.push(`/produits/${product.id}`)}
          className="flex-1 flex items-center justify-center gap-1 text-[11px] font-medium text-[#5A5A58] hover:text-[#4F46E5] py-1.5 rounded-md hover:bg-[#F7F7F5] transition-all">
        ...
        <button onClick={handleDuplicate} disabled={actionLoading === "duplicate"}
          className="flex-1 flex items-center justify-center gap-1 text-[11px] font-medium text-[#5A5A58] hover:text-[#4F46E5] py-1.5 rounded-md hover:bg-[#F7F7F5] transition-all disabled:opacity-50">
          {actionLoading === "duplicate" ? <Spinner /> : (
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <button onClick={handleArchive} disabled={actionLoading === "archive"}
          className="flex-1 flex items-center justify-center gap-1 text-[11px] font-medium text-[#5A5A58] hover:text-red-500 py-1.5 rounded-md hover:bg-red-50/50 transition-all disabled:opacity-50">
          {actionLoading === "archive" ? <Spinner /> : (
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   PRODUCTS PANE (LEFT 50%)
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

function ProductsPane({ products, loading, error, showModal, setShowModal, mutateProducts, briefTemplates }: {
  products: Product[]; loading: boolean; error: string | null;
  showModal: boolean; setShowModal: (v: boolean) => void;
  mutateProducts: () => void; briefTemplates: TemplateRow[];
}) {
  ...
  return (
    <div className="min-w-0">
      <PaneHeader
        title="Produits"
        subtitle={products.length > 0 ? `${products.length} produit${products.length !== 1 ? "s" : ""} — services, packs, digitaux` : "Gérez les services et produits que vous vendez"}
        action={
          <button onClick={() => setShowModal(true)}
            className="flex items-center gap-1.5 bg-[#4F46E5] text-white text-[13px] font-semibold px-4 py-2 rounded-lg hover:bg-[#4338CA] transition-colors whitespace-nowrap">
      ...
      {!loading && !error && products.length === 0 && (
        <motion.div className="bg-white rounded-xl border border-[#E6E6E4] py-12 px-6 text-center"
          initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <svg width="80" height="80" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="mx-auto mb-5">
          ...
          <h3 className="text-[15px] font-semibold text-[#1A1A1A] mb-1.5">Aucun produit</h3>
          <p className="text-[12px] text-[#8A8A88] mb-6 max-w-xs mx-auto">Créez votre premier produit et commencez à vendre.</p>
          <button onClick={() => setShowModal(true)}
            className="inline-flex items-center gap-1.5 bg-[#4F46E5] text-white text-[13px] font-semibold px-5 py-2.5 rounded-lg hover:bg-[#4338CA] transition-colors">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
            </svg>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   BRIEFS PANE (RIGHT 50%)
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

function BriefsPane({ templates, loading: briefsLoading, mutate: mutateBriefs }: {
  ...
  return (
    <div className="min-w-0">
      <PaneHeader
        title="Briefs"
        subtitle={briefCount > 0 ? `${briefCount} questionnaire${briefCount !== 1 ? "s" : ""} — envoyés après achat` : "Questionnaires envoyés aux clients après achat"}
        action={
          <button onClick={() => setShowCreate(true)}
            className="flex items-center gap-1.5 text-[13px] font-semibold px-4 py-2 rounded-lg bg-[#4F46E5] text-white hover:bg-[#4338CA] transition-colors whitespace-nowrap">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
            </svg>
            Créer un brief
      ...
      {briefsLoading && (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (<div key={i} className="h-20 bg-[#F7F7F5] rounded-xl animate-pulse" />))}
        </div>
      )}
      ...
      {!briefsLoading && (!templates || templates.length === 0) && (
        <motion.div className="bg-white rounded-xl border border-[#E6E6E4] py-12 px-6 text-center"
          initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#D4D4D2" strokeWidth="1.5" className="mx-auto mb-3">
            <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" />
            <rect x="9" y="3" width="6" height="4" rx="1" />
            <line x1="9" y1="12" x2="15" y2="12" /><line x1="9" y1="16" x2="13" y2="16" />
          </svg>
          <h3 className="text-[15px] font-semibold text-[#1A1A1A] mb-1.5">Aucun brief</h3>
          <p className="text-[12px] text-[#8A8A88] mb-5 max-w-xs mx-auto">Créez un questionnaire pour collecter les informations de vos clients après achat.</p>
          <button onClick={() => setShowCreate(true)}
            className="inline-flex items-center gap-1.5 bg-[#4F46E5] text-white text-[13px] font-semibold px-5 py-2.5 rounded-lg hover:bg-[#4338CA] transition-colors">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
        ...
        <div className="space-y-3">
          {templates.map((t) => (
            <motion.div key={t.id} layout
              className={`bg-white border rounded-xl transition-all ${editingId === t.id ? "border-[#4F46E5]/40 shadow-sm ring-1 ring-[#4F46E5]/10" : "border-[#E6E6E4] hover:border-[#D0D0CE]"}`}>
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                    <div className="px-4 pb-4 pt-0 space-y-4 border-t border-[#E6E6E4]">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-[#E6E6E4]">
                        <button onClick={() => setEditingId(null)} className="text-[12px] text-[#8A8A88] hover:text-[#5A5A58] transition-colors">
                          Fermer
                        </button>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   MAIN PAGE — 50/50 DUAL-PANE WORKSPACE
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
...
export default function ProduitsPage() {
  ...
  return (
    <div>
      {/* ── Desktop: 50/50 dual-pane ── */}
      <div className="hidden lg:grid lg:grid-cols-2 lg:gap-0 items-start min-h-[calc(100vh-80px)]">
        {/* LEFT PANE — Products */}
        <div className="pr-6 border-r border-[#E6E6E4]">
          <ProductsPane
            products={products} loading={productsLoading} error={productsError}
            showModal={showModal} setShowModal={setShowModal}
            mutateProducts={mutateProducts} briefTemplates={briefTemplates || []}
          />
        </div>

        {/* RIGHT PANE — Briefs */}
        <div className="pl-6">
          <BriefsPane templates={briefTemplates} loading={briefsLoading} mutate={mutateBriefs} />
        </div>
      </div>
