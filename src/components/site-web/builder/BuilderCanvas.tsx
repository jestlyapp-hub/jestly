...
export default function BuilderCanvas() {
  ...
  const handleDragOver = useCallback((e: React.DragEvent, idx: number) => {
    e.preventDefault();
    if (!activePage || dragIdx === null || dragIdx === idx) return;
    dispatch({ type: "REORDER_BLOCKS", pageId: activePage.id, fromIndex: dragIdx, toIndex: idx });
    setDragIdx(idx);
  }, [activePage, dragIdx, dispatch]);
  const handleDragEnd = () => setDragIdx(null);

  if (!activePage) {
    return (
      <div className="flex-1 flex items-center justify-center text-[14px] text-[#BBB] bg-[#F8F9FC]">
    ...
    <div className="flex-1 overflow-y-auto bg-[#F0F1F5]" ref={canvasRef}>
      ...
      >
        <div className={`${isPreview ? "" : "py-4 px-4"}`}>
          <div className={`${isPreview ? "" : "bg-white rounded-xl shadow-sm border border-[#E6E8F0] overflow-hidden"}`}>
            {activePage.blocks.length === 0 && !isPreview && (
              <div className="py-20 text-center">
                <div className="w-12 h-12 rounded-xl bg-[#F8F9FC] flex items-center justify-center mx-auto mb-3">
                ...
                <button
                  ...
                  className="text-[12px] font-semibold text-[#6a18f1] bg-[#F0EBFF] px-4 py-2 rounded-lg hover:bg-[#E5DEFF] transition-colors"
                >
                ...
                <div
                  ...
                  style={{
                    outline: isSelected
                      ? "2px solid #6a18f1"
                      : isHovered
                      ? "1px solid rgba(106,24,241,0.3)"
                      : "none",
                    outlineOffset: isSelected ? "-1px" : "0",
                    boxShadow: isSelected ? "0 0 0 4px rgba(106,24,241,0.08)" : "none",
                  }}
                >
                  ...
                  {(isSelected || isHovered) && (
                    <div className="absolute -top-5 left-2 z-20 flex items-center gap-1">
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${
                        isSelected ? "bg-[#6a18f1] text-white" : "bg-white text-[#999] border border-[#E6E8F0] shadow-sm"
                      }`}>
                  ...
                  {isSelected && (
                    <div className="absolute -top-5 right-2 z-20 flex items-center gap-0.5">
                      <button
                        ...
                        className="bg-white border border-[#E6E8F0] rounded p-1 text-[#666] hover:text-[#6a18f1] shadow-sm"
                        title="Dupliquer"
                      >
                      ...
                      <button
                        ...
                        className="bg-white border border-[#E6E8F0] rounded p-1 text-[#666] hover:text-red-500 shadow-sm"
                        title="Supprimer"
                      >
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {!isPreview && activePage.blocks.length > 0 && (
            <button
              onClick={() => setShowAddModal(true)}
              className="w-full mt-3 py-3 border-2 border-dashed border-[#E6E8F0] rounded-xl text-[12px] font-medium text-[#999] hover:border-[#6a18f1] hover:text-[#6a18f1] transition-all flex items-center justify-center gap-1.5 bg-white"
            >
