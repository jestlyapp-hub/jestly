...
export default function Process4StepsBlockEditor({ block }: { block: Extract<Block, { type: "process-4steps" }> }) {
  ...
  const addStep = () => {
    update({ steps: [...block.content.steps, { title: "Nouvelle étape", description: "Description", icon: "" }] });
  };
    ...
    <div className="space-y-3">
      ...
      {block.content.steps.map((step, i) => (
        <div key={i} className="p-2 rounded-lg border border-[#E6E6E4] space-y-2">
          ...
          <input type="text" value={step.icon ?? ""} onChange={(e) => updateStep(i, "icon", e.target.value)} placeholder="Icône (emoji ou nom)" className={inputClass} />
