      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`${toggleClass} ${hasProduct ? "bg-[#6a18f1]" : "bg-[#E6E8F0]"}`}
          >
        ...
        {hasProduct ? (
          <div className="space-y-2">
            ...
            />
            <div className="text-[10px] text-[#999] bg-[#F8F9FC] rounded-lg px-2.5 py-1.5">
              Le bouton CTA redirigera vers la page de commande du produit.
            </div>
          </div>
        ) : (
          <div>
            <label className="block text-[11px] font-medium text-[#999] mb-1">Lien du bouton</label>
            <input type="text" value={block.content.ctaLink} onChange={(e) => update({ ctaLink: e.target.value })} className={inputClass} />
          </div>
        )}
      </div>
    </div>
