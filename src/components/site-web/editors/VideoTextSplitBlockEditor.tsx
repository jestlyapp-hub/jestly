...
export default function VideoTextSplitBlockEditor({ block }: { block: Extract<Block, { type: "video-text-split" }> }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="block text-[11px] font-medium text-[#999] mb-1">URL vidéo</label>
      ...
      <div>
        <label className="block text-[11px] font-medium text-[#999] mb-1">Position de la vidéo</label>
      ...
      {block.content.ctaLabel && (
        <LinkPicker label="Lien du bouton" value={block.content.link} onChange={(link: Link) => update({ link })} />
      )}
    </div>
  );
}
