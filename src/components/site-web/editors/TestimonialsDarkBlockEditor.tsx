...
export default function TestimonialsDarkBlockEditor({ block }: { block: Extract<Block, { type: "testimonials-dark" }> }) {
  ...
  const addTestimonial = () => {
    update({
      testimonials: [...testimonials, { name: "", role: "", company: "", text: "", avatar: "", rating: 5 }],
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <label className="block text-[12px] font-medium text-[#5A5A58]">Témoignages</label>
        {testimonials.map((testimonial, index) => (
          <div key={index} className="border border-[#E6E6E4] rounded-lg p-3 space-y-2 bg-white">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[12px] font-medium text-[#5A5A58]">Témoignage {index + 1}</span>
              <button
                type="button"
            ...
            />
            <ImageUploader value={testimonial.avatar ?? ""} onChange={(url) => updateTestimonial(index, "avatar", url)} label="Avatar" previewAspect="1 / 1" />
            <div>
