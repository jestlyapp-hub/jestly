...
export default function CtaDarkGlowBlockEditor({ block }: { block: Extract<Block, { type: "cta-dark-glow" }> }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="block text-[12px] font-medium text-[#5A5A58] mb-1">Badges de confiance (un par ligne)</label>
        <textarea
          value={(block.content.trustBadges ?? []).join("\n")}
          onChange={(e) => update({ trustBadges: e.target.value.split("\n").filter((s) => s.trim()) })}
          rows={3}
          placeholder="Un badge par ligne"
