...
export default function CtaCenteredStrongBlockEditor({ block }: { block: Extract<Block, { type: "cta-centered-strong" }> }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="block text-[12px] font-medium text-[#5A5A58] mb-1">Bouton secondaire</label>
        <input type="text" value={block.content.secondaryCtaLabel ?? ""} onChange={(e) => update({ secondaryCtaLabel: e.target.value })} className={inputClass} />
      </div>
    </div>
  );
}
