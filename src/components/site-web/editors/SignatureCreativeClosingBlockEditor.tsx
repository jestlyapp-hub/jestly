...
export default function SignatureCreativeClosingBlockEditor({ block }: { block: Extract<Block, { type: "signature-creative-closing" }> }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="block text-[12px] font-medium text-[#5A5A58] mb-1">Note de signature</label>
        <textarea value={block.content.signatureNote ?? ""} onChange={(e) => update({ signatureNote: e.target.value })} rows={2} className={inputClass} />
      </div>
    </div>
  );
}
