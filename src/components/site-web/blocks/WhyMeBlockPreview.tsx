...
function WhyMeBlockPreviewInner({ content }: { content: WhyMeBlockContent }) {
  return (
    <div className="py-4">
      <h3 className="text-lg font-bold mb-3" style={{ color: "var(--site-text, #1A1A1A)" }}>{content.title}</h3>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {content.reasons.map((r, i) => (
          <div key={i} className="p-3 rounded-lg" style={{ border: "1px solid var(--site-border, #E6E6E4)" }}>
            ...
            <div className="text-[13px] font-semibold mb-0.5" style={{ color: "var(--site-text, #1A1A1A)" }}>{r.title}</div>
            <div className="text-[11px]" style={{ color: "var(--site-muted, #999)" }}>{r.description}</div>
          </div>
        ))}
      </div>
    </div>
