        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {content.rows.map((row, ri) => (
              <tr key={ri} style={{ borderTop: "1px solid var(--site-border, #E6E6E4)" }}>
                <td className="text-[12px] py-2.5 pr-4" style={{ color: "var(--site-text)" }}>{row.feature}</td>
                  ...
                  <td key={vi} className="text-center py-2.5 px-4">
                    {typeof val === "boolean" ? (
                      val ? (
                        ...
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--site-border, #ccc)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mx-auto"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
                      )
                    ) : (
                      <span className="text-[12px]" style={{ color: "var(--site-text)" }}>{val}</span>
                    )}
                  </td>
                ))}
              </tr>
