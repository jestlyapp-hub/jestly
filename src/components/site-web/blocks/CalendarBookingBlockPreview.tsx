...
function CalendarBookingBlockPreviewInner({ content }: { content: CalendarBookingBlockContent }) {
  ...
  if (content.embedUrl) {
    if (content.openModal) {
      ...
      return (
        <div className="py-6 text-center">
          ...
          <p className="text-[10px] mt-2" style={{ color: "var(--site-muted, #BBB)" }}>Ouvre la fenêtre {content.provider === "cal" ? "Cal.com" : "Calendly"}</p>
        </div>
      );
    }
    ...
    return (
      <div className="py-6">
        <h3 className="text-lg font-bold mb-1 text-center" style={{ color: "var(--site-text)" }}>{content.title}</h3>
        <p className="text-[12px] mb-4 text-center" style={{ color: "var(--site-muted)" }}>{content.description}</p>
        <div className="rounded-xl border-2 border-dashed h-48 flex flex-col items-center justify-center gap-2" style={{ borderColor: "var(--site-border, #E6E6E4)", background: "var(--site-surface, #F7F7F5)" }}>
          ...
          <span className="text-[10px] max-w-xs text-center truncate px-4" style={{ color: "var(--site-muted, #999)" }}>{content.embedUrl}</span>
        </div>
      </div>
    );
  }
