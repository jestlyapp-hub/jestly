...
export default function CalendarWorkspace() {
  ...
  // ─── Custom categories ───
  const { data: customCategories, setData: setCustomCategories } = useApi<CustomCategory[]>(
  ...
  const handleSelectEvent = useCallback((event: CalendarEvent) => {
    const isAutoGenerated = event.source !== "manual";
    if (isAutoGenerated) {
      // Événements auto-générés → drawer détail (lecture seule)
      setSelectedEvent(event);
      setDrawerOpen(true);
    } else {
      // Événements manuels → édition directe
  ...
  const handleResizeEvent = useCallback(
    (eventId: string, newStartTime: string, newEndTime: string) => {
      ...
      setEvents((prev) =>
        (prev || []).map((e) =>
          e.id === eventId ? { ...e, startTime: newStartTime, endTime: newEndTime } : e
        )
      );

      apiFetch("/api/calendar/events", {
        method: "PATCH",
        body: { id: eventId, startTime: newStartTime, endTime: newEndTime },
      }).catch((err) => {
  ...
  const handleFormSubmit = useCallback(
    async (eventData: Partial<CalendarEvent>) => {
      ...
      // Résoudre la couleur pour la mise à jour optimiste
      const resolvedColor = eventData.color
        || (eventData.categoryId
          ? (customCategories || []).find((c) => c.id === eventData.categoryId)?.color
          : undefined)
        || undefined;

      if (editingEvent) {
        // Optimistic update for edit — avec couleur résolue
        setEvents((prev) =>
          (prev || []).map((e) =>
            e.id === editingEvent.id
              ? { ...e, ...eventData, color: resolvedColor } as CalendarEvent
              : e
          )
        );
        try {
          const updated = await apiFetch<CalendarEvent>("/api/calendar/events", {
            method: "PATCH",
            body: { id: editingEvent.id, ...eventData },
          });
          // Remplacer avec la réponse API (couleur résolue côté serveur)
          setEvents((prev) =>
            (prev || []).map((ev) =>
              ev.id === editingEvent.id ? updated : ev
            )
          );
        } catch (e) {
    ...
    [editingEvent, setEvents, customCategories]
  );
  ...
  const handleSaveCategory = useCallback(
    async (cat: { id?: string; name: string; color: string; icon?: string }) => {
      if (cat.id) {
        const updated = await apiFetch<CustomCategory>("/api/calendar/categories", {
          method: "PATCH",
          body: cat,
        });
        setCustomCategories((prev) => (prev || []).map((c) => (c.id === updated.id ? updated : c)));
      } else {
        const created = await apiFetch<CustomCategory>("/api/calendar/categories", {
          body: cat,
        });
  ...
  const handleDeleteCategory = useCallback(
    async (id: string) => {
      await apiFetch("/api/calendar/categories", { method: "DELETE", body: { id } });
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-3 py-[7px] rounded-lg border border-[#E2E2E0] text-[12px] font-semibold text-[#666] hover:bg-[#F7F7F5] hover:text-[#444] transition-all cursor-pointer flex items-center gap-1.5"
          >
