...
export default function EventDetailDrawer({ event, open, onClose, onEdit, onDelete }: EventDetailDrawerProps) {
  ...
  const isAutoGenerated = isOrder || isTask || isProject || isInvoice;
  ...
  return (
    ...
    <SlidePanel open={open} onClose={onClose} title="Détail de l'événement">
      <div className="space-y-6">
        ...
        {(event.clientName || event.clientEmail || isAutoGenerated) && (
          <>
            <div className="h-px bg-[#E6E6E4]" />
            <section className="space-y-3">
              <h4 className="text-[11px] font-semibold text-[#999] uppercase tracking-wider">
              ...
              <div className="space-y-2.5">
                ...
                {isTask && event.taskId && (
                  <div className="flex items-center justify-between">
                    <span className="text-[12px] text-[#999]">Tâche</span>
                    <RelationBadge
                      type="order"
                      label="Voir la tâche"
                      href={`/taches/${event.taskId}`}
                    />
                  </div>
                )}
                {isProject && event.projectId && (
                  <div className="flex items-center justify-between">
                    <span className="text-[12px] text-[#999]">Projet</span>
                    <RelationBadge
                      type="order"
                ...
                {isInvoice && event.invoiceId && (
                  <div className="flex items-center justify-between">
                    <span className="text-[12px] text-[#999]">Facture</span>
                    <RelationBadge
                      type="order"
                ...
                {isInvoice && event.invoiceAmount != null && (
                  <DetailRow label="Montant" value={`${event.invoiceAmount} \u20AC`} />
                )}
              </div>
            </section>
          </>
        ...
        <section className="space-y-2">
          ...
          {isAutoGenerated && (
            <p className="text-[12px] text-[#999] italic">
              {isOrder && "Cet événement est généré depuis une commande. Modifiez la commande pour mettre à jour la deadline."}
              {isTask && "Cet événement est généré depuis une tâche. Modifiez la tâche pour changer l'échéance."}
              {isProject && "Cet événement est généré depuis un projet. Modifiez le projet pour changer l'échéance."}
              {isInvoice && "Cet événement est généré depuis une facture. Modifiez la facture pour changer l'échéance."}
            </p>
          )}
        </section>
      </div>
