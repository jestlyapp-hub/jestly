...
const FEATURES = [
  ...
  { key: "commandes", title: "Commandes", desc: "Gérez vos projets, statuts, livraisons et pipeline.", color: "#F59E0B", bg: "#FEF9EC", iconPath: "M9 5H2v7l6.29 6.29a1 1 0 001.42 0l5.58-5.58a1 1 0 000-1.42zM6 9a1 1 0 100-2 1 1 0 000 2", href: "/landing/commandes" },
  ...
  { key: "automations", title: "Automations", desc: "Automatisez tâches, messages, workflows et actions répétitives.", color: "#6366F1", bg: "#EEF2FF", iconPath: "M13 2L3 14h9l-1 8 10-12h-9z", href: "/landing/automations" },
...
function FeaturePreview({ feature }: { feature: typeof FEATURES[0] }) {
  const previews: Record<string, React.ReactNode> = {
    ...
    facturation: (<div className="space-y-2"><div className="p-2.5 rounded-lg" style={{ background: feature.bg }}><div className="text-[9px] text-[#8A8FA3]">Pipeline</div><div className="text-[18px] font-bold text-[#111]">2 870 €</div></div>{["Facture #042 — 1 200 €", "Facture #043 — 870 €"].map(f => (<div key={f} className="flex items-center gap-2 px-2 py-1.5 rounded" style={{ background: "#FAFAFA" }}><div className="w-1.5 h-4 rounded-full" style={{ background: feature.color }} /><span className="text-[10px] text-[#444]">{f}</span></div>))}</div>),
    ...
    automations: (<div className="space-y-2">{["Nouveau lead → Email bienvenue", "Facture payée → Notification"].map((a, i) => (<div key={a} className="flex items-center gap-2 p-2 rounded-lg" style={{ background: i === 0 ? feature.bg : "#FAFAFA" }}><div className="w-5 h-5 rounded flex items-center justify-center text-[9px]" style={{ background: feature.color, color: "white" }}>⚡</div><span className="text-[9px] text-[#444]">{a}</span></div>))}</div>),
...
/* ─── Drawers ─── */
function FeaturesDrawer({ hoveredIdx, onHover }: { hoveredIdx: number; onHover: (i: number) => void }) {
  return (
    <div className="flex p-6 gap-6" style={{ minHeight: 420 }}>
      <div className="flex-1 grid grid-cols-2 gap-x-3 gap-y-1 content-start">
        ...
        {FEATURES.map((f, i) => (
          <Link key={f.key} href={f.href} className="flex items-start gap-3 px-3 py-2.5 rounded-xl text-left transition-all duration-200" style={{ background: hoveredIdx === i ? f.bg : "transparent" }} onMouseEnter={() => onHover(i)}>
    ...
    <div className="flex p-6 gap-6">
      <div className="flex-1">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
          {SOLUTIONS.map((s) => (
            <Link key={s.title} href={s.href} className="flex items-start gap-3 px-3 py-3 rounded-xl hover:bg-[#F7F7FB] transition-colors duration-200">
  ...
  return (
    <div className="flex p-6 gap-6">
      <div className="flex-1">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
          {RESOURCES.map((r) => (
            <a key={r.title} href="#" className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-[#F7F7FB] transition-colors duration-200">
      ...
      <div className="w-[260px] flex-shrink-0 rounded-2xl p-6 flex flex-col justify-between" style={{ background: "linear-gradient(135deg, #F0FDF4, #ECFDF5)", border: "1px solid rgba(34,197,94,0.08)" }}>
        ...
        <a href="#" className="cta-gloss relative mt-4 inline-flex items-center gap-1.5 text-[12px] font-semibold text-white px-4 py-2 rounded-xl overflow-hidden transition-all duration-300 hover:-translate-y-[1px] active:scale-[0.98]" style={{ background: "linear-gradient(135deg, #16a34a, #4ade80)", boxShadow: "0 4px 14px rgba(34,197,94,0.25)" }}>
      ...
      <motion.header className="fixed top-0 left-0 right-0 z-50 flex flex-col items-center px-4 pt-5" initial={{ y: -60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.9, ease }}>
        ...
        <div className="flex items-center justify-between w-full transition-all duration-500" style={{ maxWidth: 1100, height: 64, padding: "0 24px", borderRadius: 999, background: scrolled ? "rgba(124,58,237,0.12)" : "rgba(124,58,237,0.10)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", border: "1px solid rgba(124,58,237,0.22)", boxShadow: scrolled ? "0 12px 35px rgba(0,0,0,0.10), inset 0 1px 0 rgba(255,255,255,0.45)" : "0 8px 25px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.45)" }}>
          <Link href="/landing" className="flex items-center gap-2 group" onClick={() => setActiveMenu(null)}>
            <img src="/logo-color.png" alt="Jestly" className="w-7 h-7 group-hover:scale-105 transition-transform" />
          ...
          {activeMenu && (
            <motion.div key={activeMenu} className="w-full flex justify-center mt-3" initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] as const }}>
              <div className="w-full" style={{ maxWidth: 1100, borderRadius: 28, background: "rgba(255,255,255,0.94)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "1px solid rgba(20,20,30,0.06)", boxShadow: "0 24px 80px rgba(80,70,130,0.12), 0 8px 24px rgba(0,0,0,0.06)" }}>
                {activeMenu === "features" && <FeaturesDrawer hoveredIdx={hoveredFeature} onHover={setHoveredFeature} />}
                {activeMenu === "solutions" && <SolutionsDrawer />}
                {activeMenu === "resources" && <ResourcesDrawer />}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
